...
abstract class Base extends Executable
{
    ...
    {
        return (\CronManager\Queue\Model\Process::count(["hash = :hash: AND status = :status:", 'bind' => ['hash' => $hash, 'status' => $status]]) == 1)  ? true : false;
    }
    ...
    {
        $process = \CronManager\Queue\Model\Process::findFirst(["hash = :hash:", 'bind' => $hash]);
        if (!$process) {
            $db = $this->_di->get('db');
            $db->connect();
            $process = \CronManager\Queue\Model\Process::findFirst(["hash = :hash:", 'bind' => $hash]);
