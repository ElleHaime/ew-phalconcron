...
{	
	...
	{		
		...
		switch ($type) {
			case 'model':
				...
				try {
					$params = $model::findFirst(['status = :status: AND environment = :environment:', 'bind' => ['status' => 1, 'environment' => $environment]]);
				} catch (\Exception $e) {
	...
	{
		if (!isset($this->_mainProcesses['producer']) || !$this->getJob($this->_mainProcesses['producer'])) {
			return; 
		}
	...
	{
		if (!isset($this->_mainProcesses['consumer']) || !$this->getJob($this->_mainProcesses['consumer'])) {
			return;
		}
...
*/
	...
	{
		if (!isset($this->_mainProcesses['inspector']) || !$this->getJob($this->_mainProcesses['inspector'])) {
			return;
		}
